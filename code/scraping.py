...
def auto_scrape(df_clean, save_copy=False):
    '''
    Performs a webscrape of Motortrend for new features:
    'msrp', 'fuel_type', 'weight', 'torque', 'torque_rpm', and 'horsepower'.
    '''
...
def manual_scrape(df_combo):
    '''
    Takes manually collected links and automatically scrapes Motortrend website
    for new features.
    '''
        ...
        if html.status_code != 200:
                ...
                if line.get('itemprop') != None:
                    if line.get('itemprop') == 'price':
                        df_combo.loc[df_combo['model'] == key, 'msrp'] = \
                            line.string
                    if line.get('itemprop') == 'fuelType':
                        df_combo.loc[df_combo['model'] == key,
                                     'fuel_type'] = line.string
            ...
            for line in lines_weight:
                ...
                if line.string == 'Horsepower':
                    if '@' not in line.next.next.string:
                        df_combo.loc[df_combo['model'] == key,
                                     'horsepower'] = line.next.next.string
