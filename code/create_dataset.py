...
def create_newdb():
    ...
    df.columns = ['model', 'displ', 'cyl', 'trans', 'drive', 'fuel',
                  'cert_region', 'stnd', 'stnd_description', 'underhood_id',
    ...
    df_eth_dirty[df_eth_dirty.loc[:, 'city_mpg'].isnull()] = \
        df_eth_dirty[df_eth_dirty.loc[:, 'city_mpg'].isnull()].fillna('0')
    df_gas_dirty[df_gas_dirty.loc[:, 'city_mpg'].isnull()] = \
        df_gas_dirty[df_gas_dirty.loc[:, 'city_mpg'].isnull()].fillna('0')
    df_eth_dirty[df_eth_dirty.loc[:, 'hwy_mpg'].isnull()] = \
        df_eth_dirty[df_eth_dirty.loc[:, 'hwy_mpg'].isnull()].fillna('0')
    df_gas_dirty[df_gas_dirty.loc[:, 'hwy_mpg'].isnull()] = \
        df_gas_dirty[df_gas_dirty.loc[:, 'hwy_mpg'].isnull()].fillna('0')
    df_eth_dirty[df_eth_dirty.loc[:, 'cmb_mpg'].isnull()] = \
        df_eth_dirty[df_eth_dirty.loc[:, 'cmb_mpg'].isnull()].fillna('0')
    df_gas_dirty[df_gas_dirty.loc[:, 'cmb_mpg'].isnull()] = \
        df_gas_dirty[df_gas_dirty.loc[:, 'cmb_mpg'].isnull()].fillna('0')
    df_eth_dirty[df_eth_dirty.loc[:, 'comb_co2'].isnull()] = \
        df_eth_dirty[df_eth_dirty.loc[:, 'comb_co2'].isnull()].fillna('0')
    df_gas_dirty[df_gas_dirty.loc[:, 'comb_co2'].isnull()] = \
        df_gas_dirty[df_gas_dirty.loc[:, 'comb_co2'].isnull()].fillna('0')
    df_cng_dirty[df_cng_dirty.loc[:, 'city_mpg'].isnull()] = \
        df_cng_dirty[df_cng_dirty.loc[:, 'city_mpg'].isnull()].fillna('0')
    df_gasoline_dirty[df_gasoline_dirty.loc[:, 'city_mpg'].isnull()] = \
        df_gasoline_dirty[df_gasoline_dirty.loc[:, 'city_mpg'].isnull()
                          ].fillna('0')
    df_cng_dirty[df_cng_dirty.loc[:, 'hwy_mpg'].isnull()] = \
        df_cng_dirty[df_cng_dirty.loc[:, 'hwy_mpg'].isnull()].fillna('0')
    df_gasoline_dirty[df_gasoline_dirty.loc[:, 'hwy_mpg'].isnull()] = \
        df_gasoline_dirty[df_gasoline_dirty.loc[:, 'hwy_mpg'].isnull()
                          ].fillna('0')
    df_cng_dirty[df_cng_dirty.loc[:, 'cmb_mpg'].isnull()] = \
        df_cng_dirty[df_cng_dirty.loc[:, 'cmb_mpg'].isnull()].fillna('0')
    df_gasoline_dirty[df_gasoline_dirty.loc[:, 'cmb_mpg'].isnull()] = \
        df_gasoline_dirty[df_gasoline_dirty.loc[:, 'cmb_mpg'].isnull()
                          ].fillna('0')
    df_cng_dirty[df_cng_dirty.loc[:, 'comb_co2'].isnull()] = \
        df_cng_dirty[df_cng_dirty.loc[:, 'comb_co2'].isnull()].fillna('0')
    df_gasoline_dirty[df_gasoline_dirty.loc[:, 'comb_co2'].isnull()] = \
        df_gasoline_dirty[df_gasoline_dirty.loc[:, 'comb_co2'].isnull()
                          ].fillna('0')

    # Seperate Ethanol and gas values
    df_eth_dirty.loc[:, 'city_mpg'] = \
        df_eth_dirty.loc[:, 'city_mpg'].str.split('/')
    df_eth_dirty.loc[:, 'hwy_mpg'] = \
        df_eth_dirty.loc[:, 'hwy_mpg'].str.split('/')
    df_eth_dirty.loc[:, 'cmb_mpg'] = \
        df_eth_dirty.loc[:, 'cmb_mpg'].str.split('/')
    df_eth_dirty.loc[:, 'comb_co2'] = \
        df_eth_dirty.loc[:, 'comb_co2'].str.split('/')
    df_gas_dirty.loc[:, 'city_mpg'] = \
        df_gas_dirty.loc[:, 'city_mpg'].str.split('/')
    df_gas_dirty.loc[:, 'hwy_mpg'] = \
        df_gas_dirty.loc[:, 'hwy_mpg'].str.split('/')
    df_gas_dirty.loc[:, 'cmb_mpg'] = \
        df_gas_dirty.loc[:, 'cmb_mpg'].str.split('/')
    df_gas_dirty.loc[:, 'comb_co2'] = \
        df_gas_dirty.loc[:, 'comb_co2'].str.split('/')

    # Seperate CNG and Gasoline values
    df_cng_dirty.loc[:, 'city_mpg'] = \
        df_cng_dirty.loc[:, 'city_mpg'].str.split('/')
    df_cng_dirty.loc[:, 'hwy_mpg'] = \
        df_cng_dirty.loc[:, 'hwy_mpg'].str.split('/')
    df_cng_dirty.loc[:, 'cmb_mpg'] = \
        df_cng_dirty.loc[:, 'cmb_mpg'].str.split('/')
    df_cng_dirty.loc[:, 'comb_co2'] = \
        df_cng_dirty.loc[:, 'comb_co2'].str.split('/')
    df_gasoline_dirty.loc[:, 'city_mpg'] = \
        df_gasoline_dirty.loc[:, 'city_mpg'].str.split('/')
    df_gasoline_dirty.loc[:, 'hwy_mpg'] = \
        df_gasoline_dirty.loc[:, 'hwy_mpg'].str.split('/')
    df_gasoline_dirty.loc[:, 'cmb_mpg'] = \
        df_gasoline_dirty.loc[:, 'cmb_mpg'].str.split('/')
    df_gasoline_dirty.loc[:, 'comb_co2'] = \
    ...
    df_eth_dirty.ix[df_eth_dirty['city_mpg'].map(
        ...
        df_eth_dirty[df_eth_dirty['city_mpg'].map(
            lambda x: len(x) < 2)]['city_mpg'].apply(lambda x: [0])
    df_eth_dirty.ix[df_eth_dirty['hwy_mpg'].map(
        lambda x: len(x) < 2), 'hwy_mpg'] = \
        df_eth_dirty[df_eth_dirty['hwy_mpg'].map(
            lambda x: len(x) < 2)]['hwy_mpg'].apply(lambda x: [0])
    df_eth_dirty.ix[df_eth_dirty['cmb_mpg'].map(
        lambda x: len(x) < 2), 'cmb_mpg'] = \
        df_eth_dirty[df_eth_dirty['cmb_mpg'].map(
            lambda x: len(x) < 2)]['cmb_mpg'].apply(lambda x: [0])
    df_eth_dirty.ix[df_eth_dirty['comb_co2'].map(
        lambda x: len(x) < 2), 'comb_co2'] = \
        df_eth_dirty[df_eth_dirty['comb_co2'].map(
            lambda x: len(x) < 2)]['comb_co2'].apply(lambda x: [0])
    df_gas_dirty.ix[df_gas_dirty['city_mpg'].map(
        lambda x: len(x) < 2), 'city_mpg'] = \
        df_gas_dirty[df_gas_dirty['city_mpg'].map(
            lambda x: len(x) < 2)]['city_mpg'].apply(lambda x: [0])
    df_gas_dirty.ix[df_gas_dirty['hwy_mpg'].map(
        lambda x: len(x) < 2), 'hwy_mpg'] = \
        df_gas_dirty[df_gas_dirty['hwy_mpg'].map(
            lambda x: len(x) < 2)]['hwy_mpg'].apply(lambda x: [0])
    df_gas_dirty.ix[df_gas_dirty['cmb_mpg'].map(
        lambda x: len(x) < 2), 'cmb_mpg'] = \
        df_gas_dirty[df_gas_dirty['cmb_mpg'].map(
            lambda x: len(x) < 2)]['cmb_mpg'].apply(lambda x: [0])
    df_gas_dirty.ix[df_gas_dirty['comb_co2'].map(
        lambda x: len(x) < 2), 'comb_co2'] = \
        df_gas_dirty[df_gas_dirty['comb_co2'].map(
            lambda x: len(x) < 2)]['comb_co2'].apply(lambda x: [0])
    df_cng_dirty.ix[df_cng_dirty['city_mpg'].map(
        lambda x: len(x) < 2), 'city_mpg'] = \
        df_cng_dirty[df_cng_dirty['city_mpg'].map(
            lambda x: len(x) < 2)]['city_mpg'].apply(lambda x: [0])
    df_cng_dirty.ix[df_cng_dirty['hwy_mpg'].map(
        lambda x: len(x) < 2), 'hwy_mpg'] = \
        df_cng_dirty[df_cng_dirty['hwy_mpg'].map(
            lambda x: len(x) < 2)]['hwy_mpg'].apply(lambda x: [0])
    df_cng_dirty.ix[df_cng_dirty['cmb_mpg'].map(
        lambda x: len(x) < 2), 'cmb_mpg'] = \
        df_cng_dirty[df_cng_dirty['cmb_mpg'].map(
            lambda x: len(x) < 2)]['cmb_mpg'].apply(lambda x: [0])
    df_cng_dirty.ix[df_cng_dirty['comb_co2'].map(
        lambda x: len(x) < 2), 'comb_co2'] = \
        df_cng_dirty[df_cng_dirty['comb_co2'].map(
            lambda x: len(x) < 2)]['comb_co2'].apply(lambda x: [0])
    df_gasoline_dirty.ix[df_gasoline_dirty['city_mpg'].map(
        lambda x: len(x) < 2), 'city_mpg'] = \
        df_gasoline_dirty[df_gasoline_dirty['city_mpg'].map(
            lambda x: len(x) < 2)]['city_mpg'].apply(lambda x: [0])
    df_gasoline_dirty.ix[df_gasoline_dirty['hwy_mpg'].map(
        lambda x: len(x) < 2), 'hwy_mpg'] = \
        df_gasoline_dirty[df_gasoline_dirty['hwy_mpg'].map(
            lambda x: len(x) < 2)]['hwy_mpg'].apply(lambda x: [0])
    df_gasoline_dirty.ix[df_gasoline_dirty['cmb_mpg'].map(
        lambda x: len(x) < 2), 'cmb_mpg'] = \
        df_gasoline_dirty[df_gasoline_dirty['cmb_mpg'].map(
            lambda x: len(x) < 2)]['cmb_mpg'].apply(lambda x: [0])
    df_gasoline_dirty.ix[df_gasoline_dirty['comb_co2'].map(
        lambda x: len(x) < 2), 'comb_co2'] = \
        df_gasoline_dirty[df_gasoline_dirty['comb_co2'].map(
            lambda x: len(x) < 2)]['comb_co2'].apply(lambda x: [0])

    # Grab correct value for ethanol or gas for respective tables
    df_eth_dirty.loc[:, 'city_mpg'] = \
        df_eth_dirty.loc[:, 'city_mpg'].apply(lambda x: [x[0]])
    df_eth_dirty.loc[:, 'hwy_mpg'] = \
        df_eth_dirty.loc[:, 'hwy_mpg'].apply(lambda x: [x[0]])
    df_eth_dirty.loc[:, 'cmb_mpg'] = \
        df_eth_dirty.loc[:, 'cmb_mpg'].apply(lambda x: [x[0]])
    df_eth_dirty.loc[:, 'comb_co2'] = \
        df_eth_dirty.loc[:, 'comb_co2'].apply(lambda x: [x[0]])
    df_gas_dirty.ix[df_gas_dirty['city_mpg'].map(
        lambda x: len(x) > 1), 'city_mpg'] = \
        df_gas_dirty.ix[df_gas_dirty['city_mpg'].map(
            lambda x: len(x) > 1), 'city_mpg'].apply(lambda x: [x[1]])
    df_gas_dirty.ix[df_gas_dirty['hwy_mpg'].map(
        lambda x: len(x) > 1), 'hwy_mpg'] = \
        df_gas_dirty.ix[df_gas_dirty['hwy_mpg'].map(
            lambda x: len(x) > 1), 'hwy_mpg'].apply(lambda x: [x[1]])
    df_gas_dirty.ix[df_gas_dirty['cmb_mpg'].map(
        lambda x: len(x) > 1), 'cmb_mpg'] = \
        df_gas_dirty.ix[df_gas_dirty['cmb_mpg'].map(
            lambda x: len(x) > 1), 'cmb_mpg'].apply(lambda x: [x[1]])
    df_gas_dirty.ix[df_gas_dirty['comb_co2'].map(
        lambda x: len(x) > 1), 'comb_co2'] = \
        df_gas_dirty.ix[df_gas_dirty['comb_co2'].map(
            lambda x: len(x) > 1), 'comb_co2'].apply(lambda x: [x[1]])

    # Grab correct value for CNG and Gasoline for respective tables
    df_cng_dirty.ix[df_cng_dirty['city_mpg'].map(
        lambda x: len(x) > 1), 'city_mpg'] = \
        df_gas_dirty.ix[df_gas_dirty['city_mpg'].map(
            lambda x: len(x) > 1), 'city_mpg'].apply(lambda x: [x[0]])
    df_cng_dirty.ix[df_cng_dirty['hwy_mpg'].map(
        lambda x: len(x) > 1), 'hwy_mpg'] = \
        df_gas_dirty.ix[df_gas_dirty['hwy_mpg'].map(
            lambda x: len(x) > 1), 'hwy_mpg'].apply(lambda x: [x[0]])
    df_cng_dirty.ix[df_cng_dirty['cmb_mpg'].map(
        lambda x: len(x) > 1), 'cmb_mpg'] = \
        df_gas_dirty.ix[df_gas_dirty['cmb_mpg'].map(
            lambda x: len(x) > 1), 'cmb_mpg'].apply(lambda x: [x[0]])
    df_cng_dirty.ix[df_cng_dirty['comb_co2'].map(
        lambda x: len(x) > 1), 'comb_co2'] = \
        df_gas_dirty.ix[df_gas_dirty['comb_co2'].map(
            lambda x: len(x) > 1), 'comb_co2'].apply(lambda x: [x[0]])
    df_gasoline_dirty.ix[df_gasoline_dirty['city_mpg'].map(
        lambda x: len(x) > 1), 'city_mpg'] = \
        df_gasoline_dirty.ix[df_gasoline_dirty['city_mpg'].map(
            lambda x: len(x) > 1), 'city_mpg'].apply(lambda x: [x[1]])
    ...
    df_eth_dirty.loc[:, 'comb_co2'] = \
        df_eth_dirty.loc[:, 'comb_co2'].apply(lambda x: x[0])
    df_gas_dirty.loc[:, 'city_mpg'] = \
        df_gas_dirty.loc[:, 'city_mpg'].apply(lambda x: x[0])
    df_gas_dirty.loc[:, 'hwy_mpg'] = \
        df_gas_dirty.loc[:, 'hwy_mpg'].apply(lambda x: x[0])
    df_gas_dirty.loc[:, 'cmb_mpg'] = \
        df_gas_dirty.loc[:, 'cmb_mpg'].apply(lambda x: x[0])
    df_gas_dirty.loc[:, 'comb_co2'] = \
        df_gas_dirty.loc[:, 'comb_co2'].apply(lambda x: x[0])
    df_cng_dirty.loc[:, 'city_mpg'] = \
        df_cng_dirty.loc[:, 'city_mpg'].apply(lambda x: x[0])
    df_cng_dirty.loc[:, 'hwy_mpg'] = \
        df_cng_dirty.loc[:, 'hwy_mpg'].apply(lambda x: x[0])
    df_cng_dirty.loc[:, 'cmb_mpg'] = \
        df_cng_dirty.loc[:, 'cmb_mpg'].apply(lambda x: x[0])
    df_cng_dirty.loc[:, 'comb_co2'] = \
        df_cng_dirty.loc[:, 'comb_co2'].apply(lambda x: x[0])
    df_gasoline_dirty.loc[:, 'city_mpg'] = \
        df_gasoline_dirty.loc[:, 'city_mpg'].apply(lambda x: x[0])
    df_gasoline_dirty.loc[:, 'hwy_mpg'] = \
        df_gasoline_dirty.loc[:, 'hwy_mpg'].apply(lambda x: x[0])
    df_gasoline_dirty.loc[:, 'cmb_mpg'] = \
        df_gasoline_dirty.loc[:, 'cmb_mpg'].apply(lambda x: x[0])
    df_gasoline_dirty.loc[:, 'comb_co2'] = \
        df_gasoline_dirty.loc[:, 'comb_co2'].apply(lambda x: x[0])

    # Convert to correct type
    df_eth_dirty[['city_mpg', 'hwy_mpg', 'cmb_mpg', 'comb_co2']] = \
        df_eth_dirty[['city_mpg', 'hwy_mpg', 'cmb_mpg', 'comb_co2']
                     ].astype(int64)
    df_gas_dirty[['city_mpg', 'hwy_mpg', 'cmb_mpg', 'comb_co2']] = \
        df_gas_dirty[['city_mpg', 'hwy_mpg', 'cmb_mpg', 'comb_co2']
                     ].astype(int64)
    df_cng_dirty[['city_mpg', 'hwy_mpg', 'cmb_mpg', 'comb_co2']] = \
        df_cng_dirty[['city_mpg', 'hwy_mpg', 'cmb_mpg', 'comb_co2']
                     ].astype(int64)
    df_gasoline_dirty[['city_mpg', 'hwy_mpg', 'cmb_mpg', 'comb_co2']] = \
        df_gasoline_dirty[['city_mpg', 'hwy_mpg', 'cmb_mpg', 'comb_co2']
                          ].astype(int64)

    # Combine all df, drop fuel type, and sort newly formed df
    df_dirty = pd.concat([df,
                          df_eth_dirty,
                          df_gas_dirty,
                          df_cng_dirty,
                          df_gasoline_dirty])
...
def cat_conv(df_clean):
    '''
    Converts several of the dataframe's features into categorical variables a
    model can handle.
    '''

    # Convert 'smartway' feature to int categorical
    smartway_conv = {'No': 0, 'Yes': 1, 'Elite': 2}
    df_clean.loc[:, 'smartway'] = \
        df_clean.loc[:, 'smartway'].map(smartway_conv)
    ...
    trans_speed_conv = {'AMS-6': 6,
                        ...
                        'SemiAuto-8': 8,
                        'SemiAuto-9': 9}
    df_clean.loc[:, 'trans_speed'] = \
        df_clean.loc[:, 'trans'].map(trans_speed_conv)
    ...
    df_clean.loc[:, 'cert_region'] = \
        df_clean.loc[:, 'cert_region'].map(cert_region_conv)
    ...
    veh_class_conv = {'small car': 0,
                      ...
                      'van': 8,
                      'minivan': 9}
    df_clean.loc[:, 'veh_class'] = \
        df_clean.loc[:, 'veh_class'].map(veh_class_conv)
...
def merge_auto_scrape(df_clean, saved_copy=False):
    '''
    ...
    s_temp['model'] = zip(s_temp.index.get_level_values(0),
                          s_temp.index.get_level_values(1))
    ...
    df_combo = df_clean.merge(s_temp,
                              how='left',
                              left_on='model',
                              right_on='model')
    ...
    # Set correct values
    df_combo['weight'] = \
        df_combo[df_combo['weight'].notnull()]['weight'].apply(
            lambda x: x[:-5])
    df_combo[df_combo['weight'] == ''] = np.nan
    df_combo['msrp'] = \
        df_combo[df_combo['msrp'].notnull()]['msrp'].apply(
    ...
    fuel_type_conv = {'Unleaded Regular': 0,
                      ...
                      'Unleaded Premium': 2,
                      'Diesel': 3}
    df_combo.loc[:, 'fuel_type'] = \
        df_combo.loc[:, 'fuel_type'].map(fuel_type_conv)
            ...
            for line in lines_price:
                if line.get('itemprop') != None:
                    if line.get('itemprop') == 'price':
                        df_combo.loc[df_combo['model'] == key2,
                                     'msrp'] = str(line.string)
                    if line.get('itemprop') == 'fuelType':
                        df_combo.loc[df_combo['model'] == key2,
                                     'fuel_type'] = str(line.string)
            ...
            for line in lines_weight:
                ...
                if line.string == 'Horsepower':
                    if '@' not in line.next.next.string:
                        df_combo.loc[df_combo['model'] == key2,
                                     'horsepower'] = str(line.next.next.string)
    ...
    df_combo_slim = df_combo.drop(op_doubles)

    # Save filled df to local pickle
    if saved_copy is True:
